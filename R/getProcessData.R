...
process_raw_data <- function(sample_info,
    ...
    # We're going to log, for each sample, what needed to be done, what was
    # tried and the result of that attempt. This will make it easier to see what
    # we may need to fix or retry. At the end I'll write this out as a text file
    # next to the sample_info sheet input. I'm going to store this in the
    # sample_data tibble().
    # We're also going to store the cutoffs generated by auto_subset() in this.
    ...
    if (nrow(to_download) > 0) {
        ...
        to_download$download_worked <-
            ...
            unlist()
        # Merge to_download back into sample_data to include information on if
        #  the download was successful
        shared_cols <- intersect(colnames(to_download), colnames(sample_data))
        sample_data <-
    ...
    to_count_tbl_list <-
        parallel::mclapply(seq_len(length(to_count_tbl_list)),
                           mc.cores = 100,
                           function(i) {
            sub_data <- to_count_tbl_list[[i]]
            fastq_folders <-
                paste0(fastq_folder, "/",
                       sub_data$Sample_Project,
                       lapply(sub_data$Protocol,
                              function(prot) {
                                switch(prot,
                                       "3GEX" = "",
                                       "MGEX" = "_R",
            ...
            if (any(!dir.exists(fastq_folders))) {
                big_problem(paste("Fastq folder does not exist for project",
                                  sub_data$Sample_Project[1],
                                  ". Skipping."))
                sub_data$cr_count_done <- FALSE
                return(sub_data)
            }
            ...
            return_value <-
                cellranger_count(sample_info = sub_data,
                                 email = email,
                                 counts_folder = counts_folder,
                                 fastq_folder = fastq_folder,
    ...
    sample_data <-
        dplyr::left_join(sample_data,
                         dplyr::bind_rows(to_count_tbl_list),
                         by = shared_cols)
        ...
        parallel::mclapply(unique(to_make_sobj$Sample_ID),
            ...
            if (!dir.exists(paste0(counts_folder, "/",
                ...
                big_problem(paste("Counts folder does not exist for sample",
                                  s_id,
                                  ". Skipping."))
                to_make_sobj$sobj_made[to_make_sobj$Sample_ID == s_id] <-
                    "Counts folder does not exist."
                return(FALSE)
            }
    ...
    # Write out sample_data to a text file next to the sample_info sheet
    # Include the date and time in the file name
    readr::write_tsv(sample_data,
                     paste0(sample_info,
                            "_",
